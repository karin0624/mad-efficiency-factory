...
## ADR 0001: Miner・Smelter フットプリント 1x1、出力ポート offset=(0,0)
...
func test_register_miner_creates_output_port_north() -> void:
	# 採掘機(ID=1, 1x1)を北向きで (2,2) に配置
	# 出力ポート: base_cell(2,2) + rotate_offset((0,0),(1,1),N)=(0,0) = world(2,2), dir=S
...
func test_register_miner_output_world_position_east() -> void:
	# 採掘機を東向きで (2,2) に配置
	# 出力ポート北向き定義: offset=(0,0), dir=S (1x1)
	# 東向き回転: rotate_offset((0,0),(1,1),E) = (1-1-0, 0) = (0, 0)
...
func test_register_miner_output_world_position_south() -> void:
	# 採掘機を南向きで (2,2) に配置
	# 南向き回転: rotate_offset((0,0),(1,1),S) = (1-1-0, 1-1-0) = (0,0)
	# world_pos = (2,2) + (0,0) = (2,2)
...
func test_register_miner_output_world_position_west() -> void:
	# 採掘機を西向きで (2,2) に配置
	# 西向き回転: rotate_offset((0,0),(1,1),W) = (0, 1-1-0) = (0,0)
	# world_pos = (2,2) + (0,0) = (2,2)
