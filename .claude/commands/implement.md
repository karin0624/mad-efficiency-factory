...
5. ローカルとリモートの差分を確認:
   ...
   - **両方 0** の場合: そのまま続行
   - **behind > 0（リモートが先行）** の場合: 「リモートに {N} 件の新しいコミットがあります。`git pull` してから続行しますか？」とユーザーに確認
     - ユーザーが承認した場合: `git pull origin <base-branch>` を実行して続行
     - ユーザーが拒否した場合: 現在の状態のまま続行
   - **ahead > 0（ローカルが先行）** の場合: 「ローカルに {N} 件の未プッシュコミットがあります。`git push` してから続行しますか？」とユーザーに確認
     - ユーザーが承認した場合: `git push origin <base-branch>` を実行して続行
     - ユーザーが拒否した場合: 現在の状態のまま続行
