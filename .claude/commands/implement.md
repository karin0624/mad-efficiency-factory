...
2. ブランチ/worktreeが既に存在するか確認:
   - `.claude/worktrees/feat/<sanitized-plan-name>` にworktreeが存在する場合:
     ...
     b. `spec.json.phase` で再開ポイントを判定:
        - `validated` → Agent A, B, B2をスキップ → Agent C（commit）から開始
        - `impl-completed` → Agent A, Bをスキップ → Agent B2（validate-impl）から開始
...
- **Glob**: planファイルの解決
- **Agent**: サブエージェントA、B、B2、C（、D）を順次起動（`isolation: "worktree"` は使用しない — worktreeは事前作成済み）
...
- **Agent A/B/B2失敗**: どのAgentが失敗したか報告し、手動リカバリのためにworktreeを保持
