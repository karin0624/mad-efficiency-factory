...
3. **要件の生成**（メタ情報の収集を同時に行う）:
   ...
   - **メタ情報の収集**（要件生成と並行して、以下の情報を内部的に蓄積する）:
     - **AI補完箇所**: Plan の記述が不十分で AI が補完した箇所（要件ID + 補完内容 + Plan の元の記述）
     - **テストレイヤー分類の迷い**: テストレイヤー分類で候補が複数あった箇所（要件ID + 候補レイヤー + 最終判断理由）
     - **スコープ境界判断**: 各機能のスコープ判断（IN / OUT / DEFERRED）とその理由

4. **レビュー文書の生成**:
   - ステップ 3 で収集したメタ情報を `.kiro/specs/$1/requirements-review.md` として出力する
   - 以下の構造で生成する:
   ...
   ### 🔴 要確認（Plan で曖昧 → AI が補完した領域）
   <!-- 要件ID ごとに: 補完内容 / Plan の元の記述 / 確認ポイント -->
   <!-- AI が補完した箇所がない場合は「該当なし」と記載 -->

   ### 🟡 判断が必要（トレードオフ）
   <!-- テストレイヤー分類で迷った箇所: 要件ID / 候補レイヤー / 最終判断理由 -->
   <!-- スコープ境界の判断が曖昧だった箇所 -->

   ### 🟢 参考
   <!-- スコープ境界 (IN / OUT / DEFERRED) の一覧 -->
   <!-- 要件全体構成と Layer 分布 -->

   ## 要件パラフレーズ
   <!-- 各 EARS 要件の自然言語での言い換え -->
...
5. **メタデータの更新**:
   ...
   - `review.requirements_generated: true` を設定する
...
2. **ドキュメントステータス**: requirements.md、requirements-review.md、spec.json のメタデータが更新されたことを確認する
3. **レビューハイライト**: requirements-review.md の 🔴 要確認項目の件数を表示する（例: "レビュー文書に2件の要確認事項があります"）
4. **次のステップ**: requirements-review.md を確認してから承認して進めるか、修正するかのガイダンスを提示する
