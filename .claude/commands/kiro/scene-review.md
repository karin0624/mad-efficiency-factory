...
allowed-tools: Read, Write, Edit, Glob, Grep, screenshot-game-view, screenshot-scene-view, Unity_ManageScene, AskUserQuestion
...
- **Mission**: Process uncompleted Human Review sub-tasks from tasks.md by capturing screenshots, presenting review criteria, and marking tasks complete upon human approval
- **Success Criteria**:
  - All pending Human Review sub-tasks from tasks.md identified
  - Screenshots captured for each review item
...
For each pending Human Review task:
1. **Use `screenshot-game-view`** to capture current Game View
2. If the review criteria reference a specific scene, use `Unity_ManageScene` to open it first
3. Optionally use `screenshot-scene-view` for editor-perspective layout review

### Step 4: Present Review Items

For each pending Human Review task, present to the user:
- **Task ID and description** (from tasks.md)
- **Screenshot(s)** captured in Step 3
...
- **Screenshot-first**: Always capture fresh screenshots; do not rely on cached images
- **No auto-pass**: Never mark Human Review tasks as complete without explicit user approval
</instructions>

## Tool Guidance
- **Unity MCP**: Use screenshot-game-view and screenshot-scene-view for captures
...
2. **Screenshots**: Game View captures for each review item
...
**Format**: Concise, screenshot-centric. Under 300 words of text; screenshots do the heavy lifting.
...
- Stop with guidance to configure Unity MCP in `.mcp.json`
...
**No Scene Open**:
- Warn and attempt to open default scene, or ask user which scene to review
