...
### ステップ 2.5: アーティファクトの自動読み込み（E2Eテスト保存済みスクショ・セマンティックダンプの活用）

**E2Eテストで保存されたアーティファクトの確認**:
- `godot/test_screenshots/` ディレクトリを確認し、利用可能なスクショファイルを一覧表示する
- `godot/test_snapshots/` ディレクトリも確認し、セマンティックダンプファイルを一覧表示する
- `.txt` ファイルはReadツールで読み込み、ゲーム状態をユーザーに提示する
- 各レビュータスクに関連するスクショが存在する場合、Readツールで自動読み込みしてユーザーに提示する
- これによりユーザーはゲームを手動実行せずに視覚的・構造的な評価が可能になる
- アーティファクトが存在しない場合は、手動確認手順をユーザーに提示する
