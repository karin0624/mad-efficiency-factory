...
- 純粋関数のみ、状態を持たない
- 正方形グリッドの汎用回転公式（MVPの1x1機械ではidentity変換、将来の大型機械にも対応）
...
- machine_size は EntityDefinition.footprint と一致しなければならない（フットプリントの単一権限源は EntityDefinition）
- local_direction はポートが機械外部へ「向いている方向」を示す。出力ポートは local_direction 方向の隣接セルへアイテムを押し出し、入力ポートは local_direction 方向の隣接セルからアイテムを引き込む
...
|---------|---------|------|-------------|--------------|
| Miner | 1 | 1x1 | なし | offset=(0,0), dir=S |
| Smelter | 2 | 1x1 | offset=(0,0), dir=N | offset=(0,0), dir=S |
...
| 重複 entity_id の登録 | 登録を拒否（警告ログ）、既存状態を維持 | 保存 |
...
1. **E2E統合フロー**: 出力ポート付き機械 → ベルト5本 → 入力ポート付き機械のアイテム到達検証
   - **標準レイアウト例（1x1機械、北向き）**: Miner@(5,0) dir=N → 出力ポート(5,0) dir=S → Belt@(5,1)〜(5,5) 南方向 → Smelter@(5,6) dir=N → 入力ポート(5,6) dir=N が Belt@(5,5) と接続
...
| 2026-03-17 | [DATA] | MVP Port Configurations: Miner・Smelter の machine_size を 2x2→1x1 に変更、出力ポート offset を (1,1)→(0,0) に変更。PortMath 説明文を更新 | ADR 0001: Miner・Smelter フットプリント 2x2→1x1 変更に伴うデータ値更新 |
| 2026-03-17 | [INTERFACE] | MachinePortGrid に `get_port_index(port: Dictionary) -> int` と `is_dirty() -> bool` を追加 | テスト支援ユーティリティ。`get_port_index` はポートDictionaryからインデックスを逆引き、`is_dirty` はdirtyフラグの検証に使用。設計契約（Service/State）には影響なし |
