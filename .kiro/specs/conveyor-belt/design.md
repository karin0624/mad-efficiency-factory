...
1. **アイテム移動の滑らかさ**: ベルト上のアイテムがカクつかず滑らかに移動すること
   - レビュー方法: 直線5本ベルトチェーンでアイテム投入し目視確認
   - 受け入れ閾値: 60FPSで視覚的に滑らかな移動
2. **バックプレッシャーの視覚フィードバック**: 停止と移動の状態が視覚的に区別できること
   - レビュー方法: 出力先を塞いだ状態でベルトチェーンを目視確認
   - 受け入れ閾値: 停止/移動の状態遷移が即座に識別できる
3. **大量ベルトの描画パフォーマンス**: 500本以上のベルトが配置された状態でのフレームレート
   - レビュー方法: 500本のベルトを配置してFPSカウンターを確認
   - 受け入れ閾値: 30FPS以上を維持
