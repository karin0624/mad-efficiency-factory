...
- **Selected pattern**: レイヤードアーキテクチャ（Data Layer → Core Logic → Presentation）— ハイブリッドデータ指向方針に準拠
...
var footprint: Vector2i  ## フットプリントサイズ（例: Vector2i(1, 1)）


func is_valid() -> bool
## Postconditions: id > 0, display_name != "", footprint.x > 0, footprint.y > 0 を全て満たす場合true
```
...
## Postconditions: 全配置済みPlacedEntityの配列を返す（読み取り専用）
...
- PlacementInputNodeが`entity_placed` / `entity_removed`シグナルを提供し、外部システムとの連携に使用する。
...
- **PlacementSystem.remove()**: 複数セルエンティティの任意セル指定による撤去
...
| 日付 | カテゴリ | 変更内容 |
|------|---------|---------|
| 2026-03-17 | [INTERFACE] | EntityDefinition に `is_valid()` メソッドを追加（前提条件の検証用） |
| 2026-03-17 | [INTERFACE] | PlacementSystem に `get_all_entities()` メソッドを追加（全配置済みエンティティ取得用） |
