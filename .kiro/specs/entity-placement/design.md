...
sequenceDiagram
    ...
    alt 配置成功
        ...
        PS-->>Input: PlaceResult.OK
    else 配置失敗（占有済みまたは範囲外）
        CG-->>PS: false
        PS-->>Input: PlaceResult.BLOCKED / OUT_OF_BOUNDS
    end
```
...
sequenceDiagram
    ...
    alt エンティティ存在
        ...
        PS-->>Input: RemoveResult.OK
    else エンティティなし
        CG-->>PS: 0
        PS-->>Input: RemoveResult.NO_ENTITY
    end
```
...
## Postconditions: Miner(2x2), Smelter(2x2), Belt(1x1), DeliveryBox(1x1)が登録済み
```
...
- **PlacementSystem.remove()**: 2x2エンティティの任意セル指定による撤去
...
| 日付 | カテゴリ | 変更内容 |
|------|---------|---------|
| 2026-03-14 | [ARCHITECTURE] | GhostPreviewNode: Sprite2D/modulate方式からNode2D._draw()+draw_rect()+Color定数方式に変更。同等の視覚効果をよりシンプルに実現。 |
| 2026-03-14 | [INTERFACE] | PlacementInputNode: `entity_placed(entity_id, pos, direction, entity_type_id)` / `entity_removed(entity_id, pos, entity_type_id)` シグナルを追加。Presentation↔外部連携用。 |
| 2026-03-14 | [INTERFACE] | PlacementInputNode: `placement_system._registry` への直接アクセスが存在（エンティティ定義のフットプリント取得のため）。PlacementSystemの公開APIに定義参照メソッドが不足している点を記録。 |
