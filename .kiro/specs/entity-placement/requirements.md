...
対象エンティティ（MVP）: Miner(1x1), Smelter(1x1), Belt(1x1), DeliveryBox(1x1)。配置対象領域は64x64セルのグリッドとする。
...
3. When 矩形フットプリント（幅w×高さh, w>1またはh>1）のエンティティが基準セル(x, y)に配置される, the PlacementSystem shall (x,y)から(x+w-1, y+h-1)までの全セルを占有状態に設定する
...
4. When 基準セル(63, 63)に1x1エンティティの配置が試みられ、そのセルが空きの場合, the PlacementSystem shall 配置を許可する
...
2. When 複数セルを占有するエンティティのいずれか1セルが指定される, the PlacementSystem shall 当該エンティティ全体を撤去する
...
#### Acceptance Criteria
1. The PlacementSystem shall Minerのフットプリントを1x1として扱う
2. The PlacementSystem shall Smelterのフットプリントを1x1として扱う
