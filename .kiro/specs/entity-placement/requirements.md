...
3. When 複数セルを占有するフットプリント（例: 2x2）のエンティティが基準セル(x, y)に配置される, the PlacementSystem shall フットプリントが占有する全セルを占有状態に設定する（例: 2x2フットプリントの場合、(x,y), (x+1,y), (x,y+1), (x+1,y+1)の4セル）
